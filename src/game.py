...
class Game:
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # стрельба по пробелу (только в игре)
                if event.key == pygame.K_SPACE and self.game_state == "PLAYING":
                    if self.player and hasattr(self.player, 'shoot'):
                        ...
                        bullet = self.player.shoot(mouse_pos_temp[0], mouse_pos_temp[1])
                        if bullet:
                            self.bullets.append(bullet)

            # клик мыши для стрельбы (только в игре)
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and self.game_state == "PLAYING":
                    if self.player and hasattr(self.player, 'shoot'):
                        mouse_pos_click = pygame.mouse.get_pos()
                        bullet = self.player.shoot(mouse_pos_click[0], mouse_pos_click[1])
                        if bullet:
                            self.bullets.append(bullet)

            # Обработка событий меню (кнопки)
            if self.game_state == "MENU":
                result = self.menu.handle_event(event, mouse_pos)
                if result == "start":
                    self.start_game()
                elif result == "exit":
                    self.running = False
    ...
    def update(self, dt):
        ...
        # Обновляем меню (для анимации звезд)
        if self.game_state == "MENU":
            self.menu.update(dt)
            return
