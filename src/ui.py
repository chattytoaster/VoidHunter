...
class Menu:
    ...
    def set_screen_size(self, width, height):
        ...
        self.stars = [MovingStar(width, height) for _ in range(150)]

    def handle_event(self, event, mouse_pos):
        if event.type == pygame.MOUSEMOTION:
            start_button = pygame.Rect(self.screen_width // 2 - 100, self.screen_height // 2 - 30, 200, 50)
            exit_button = pygame.Rect(self.screen_width // 2 - 100, self.screen_height // 2 + 40, 200, 50)
            self.start_hover = start_button.collidepoint(mouse_pos)
            self.exit_hover = exit_button.collidepoint(mouse_pos)
